	...
	if (_isSoldier && !_exit) then 
	{
		...
		if (_reinforcement == "REINFORCEMENT" || _rfid > 0) then {
			...
			if ( (KRON_UPS_reinforcement || _rfidcalled) && (!_reinforcementsent)) then {				
				...
				if (KRON_UPS_Debug>0) then {player sidechat format["reinforcement move to %1",_fixedtargetPos]};
...
////////////////////////////////////////// TARGET FOUND ////////////////////////////////////////////////////////////////
	...
	//////////// ARTI REACTION ///////////////////////////////////////////////////////////
		...
		_artillerysideunits = call (compile format ["KRON_UPS_ARTILLERY_%1_UNITS",_side]);
			...
			If ((_supressed || morale _npc < -0.6 || (count _members >= (count (units _npc)) - _nbrwoundedunit || _nbrTargets > (count units _npc) *2)) && !_reinforcementsent && _reinforcement == "REINFORCEMENT") then 
			{
				_reinforcementsent=[_npc,_target,_radiorange,_side] call Aze_Askrenf;
				if (KRON_UPS_Debug>0) then {player sidechat format["%1 ask for reinforcement move to %2",_grpid, getpos _target]};
			};
...
// **********************************************************************************************************************
	if (_fightmode == "walk" && !_ambush && !_nowp && !_targetsnear && !_fortify && _nomove != "NOMOVE" && !_newpos) then 
	{
		...
		If (_Behaviour != "COMBAT") then
		{
			...
			if (!_swimming
			&& (( _targetdist <= (_area/2) || moveToFailed _npc) && (_timeontarget > KRON_UPS_maxwaiting))) then 
			{
				_makenewtarget=true;
				_Behaviour = _orgMode;
			};
