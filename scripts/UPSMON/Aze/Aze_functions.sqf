...
MON_Bld_remove = ["Bridge_PathLod_base_F","Land_Slum_House03_F","Land_Bridge_01_PathLod_F","Land_Bridge_Asphalt_PathLod_F","Land_Bridge_Concrete_PathLod_F","Land_Bridge_HighWay_PathLod_F","Land_Bridge_01_F","Land_Bridge_Asphalt_F","Land_Bridge_Concrete_F","Land_Bridge_HighWay_F","Land_Canal_Wall_Stairs_F","Land_dp_bigTank_F","Land_Metal_Shed_F"];
...
MON_moveBuildings2 = {
	...
	if ( _all && count _units > 0 ) then {
		If (1 in _blds) then {_blds = _blds - [1];};
		_blds2 = [];
		_minpos = _minpos;
		{
...
MON_movetoBuilding2 = {
	...
	if (_time > 1) exitwith {_npc domove getposATL (leader group _npc);};
