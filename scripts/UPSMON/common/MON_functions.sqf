...
MON_doStop = {
	...
	if 	((_npc getvariable "UPS_FORTIFY") select 1) then 
	{	
		dostop _npc ;
...
MON_doParadrop = {
	...
	{			
		(driver _heli) flyInHeight _flyingheigh;
	...
	_crew = crew _heli;
	//((position _heli) select 2) >= 60 && 
	if (!surfaceIsWater _helipos && (!canMove _heli || (toUpper (behaviour _npc) IN _landonBeh))) then 
	{			
		    //[_heli] spawn MON_landHely;				
			[_heli,_startpos,_targetPos] spawn MON_landHely;
	} 
	else 
	{				
...
MON_landHely = {
	...
	_timeout = time + 160; 
	waituntil {!alive _heli || time > _timeout || ((abs(velocity _heli select 2)) <= 2 && ((position _heli) select 2) < 3)};
	...
	if (((position _heli) select 2) > 2 && ((position _heli) select 2) < 30 && !surfaceiswater position _heli) then { 		
		...
		_heli land "GET OUT";
		_timeout = 120 + time;
		//waituntil {!alive _heli || time > _timeout || ( (abs(velocity _heli select 2)) <= 2 && ((position _heli) select 2) <= 0.7)};	
		  waituntil {!alive _heli || time > _timeout || ( (abs(velocity _heli select 2)) <= 2 && ((position _heli) select 2) <= 5)};	
		
		if (KRON_UPS_Debug>0 ) then { player globalchat format["landing: time: %1 velocity: %2 High: %3",time > _timeout,(abs(velocity _heli select 2)),(position _heli) select 2];};
...
MON_GetNearestBuildings = {
	...
	_OCercanos = [ (nearestObjects [_object, ["house","building"], _distance]), { _x call Aze_filterbuilding } ] call BIS_fnc_conditionalSelect;
	
	
	{
		_bldpos = [_x, 20] call BIS_fnc_buildingPositions;
		sleep 0.2;
		_allpos = [_x,_bldpos,"RANDOMA"] call Aze_SortOutBldpos2; 
		if (damage _x <= 0 ) then { _bldpositions = _bldpositions + [[_x,_allpos]];};
